...
router.post("/editUser", async (req, res, next) => {
  ...
  if (data.id) {
    let sql = `update user set name='${data.name}',age='${data.age}',password='${data.password}',avatar='${data.avatar}' where id='${data.id}'`;
    let result = await sqlHandle.DB2(sql);
    if (result.affectedRows == 1) {
      res.send(commonJS.outPut(200, data, "success"));
    } else {
      res.send(commonJS.outPut(500, result, "failxxx"));
    }
  } else {
    res.send(commonJS.outPut(500, "用户id不存在"));
  }
});
